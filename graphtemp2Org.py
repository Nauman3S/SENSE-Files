...
#added by me##Konstant=sin(2*pi*sin(30))
T0=1.0
T1=Konstant
...
def SinwaveformGenerator(arg):
  ...
  if len(values)%100>70:
    #dd=0
    #values.append(cos(2*pi*fcos*Ta))
    ...
    values.append(Tnext)
    print(values[len(values)-1])
    try:###logic for memory clearance and distortion elimination
        if(len(values)>10):
...
def RealtimePloter(arg):
  ...
  ax.axis([CurrentXAxis.min(),CurrentXAxis.max(),-1.5,1.5])#-1.5 to 1.5
  manager.canvas.draw()
  #manager.show()
